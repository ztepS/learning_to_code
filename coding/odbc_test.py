...
#    print flatL0
    ...
    for i in combinationList:
        ...
        try: L0SumList.append(i[0].L0+i[1].L0+i[2].L0)
        except TypeError: print i[0].L0, i[1].L0, i[2].L0
...
#        if(abs(deltaSumList[i])<0.01 and L0SumList[i] > minL0 and L0SumList[i]< maxL0): combinedList.append(i)
    ...
    groupList=[]  
    #groupList = shelve.open("grouplist.tmp", writeback=True)
    
    
    print(len(combinationList)*len(sphereList))
    
    doableSize=len(combinationList)
    if(len(combinationList)>350000):doableSize=100000
    
    for i in range(doableSize):
        #if(int(i/1000)==i/1000):print i, " of ", doableSize
        for j in range(len(sphereList)):
            L0TotalSum=L0SumList[i] + sphereList[j].L0
            deltaTotalSum = deltaSumList[i] + sphereList[j].delta
            if (L0TotalSum>=minL0 and L0TotalSum<=maxL0 and abs(deltaTotalSum) <= maxDelta and combinationList[i][1] is not combinationList[i][2]): 
...
#                print debugLList[combinedList[i]], "<--L"
                ...
                try: groupList.append(Group())
                except(MemoryError): 
                    print "Memory error: i, j = ", i, j, len(groupList)
                    return 0
...
#                groupList[len(groupList)-1].report()
            #else: print L0TotalSum, deltaTotalSum
        del combinationList[i]
    print len(groupList)
